...
class ProyectosRepository {
  ...
  async create(proyecto) {
    const { rows } = await db.query(
      'INSERT INTO tipo_proyecto (id, descripcion) VALUES ($1, $2) RETURNING *',
  ...
  async update(id, proyecto) {
    const { rows } = await db.query(
      'UPDATE tipo_proyecto SET id = $1, descripcion = $2 WHERE id = $3 RETURNING *',
      [proyecto.codigo, proyecta.descripcion, id]
    );
    if (rows.length === 0) return null;
    const row = rows[0];
    return new tipoProyecto(
